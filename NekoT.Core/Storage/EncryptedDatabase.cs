...
public class EncryptedDatabase : IAsyncDisposable
{
    ...
    public async Task InitializeAsync()
    {
        ...
        createTableCmd.CommandText = @"
            CREATE TABLE IF NOT EXISTS Usage (
                Id TEXT PRIMARY KEY, Model TEXT NOT NULL, PromptTokens INTEGER NOT NULL,
                CompletionTokens INTEGER NOT NULL, TotalTokens INTEGER NOT NULL,
                Timestamp TEXT NOT NULL, Source TEXT, EncryptedData BLOB);
    ...
    private async Task InitializeSaltAsync()
    {
        ...
        if (result != null && int.TryParse(result.ToString(), out var version) && version >= 2)
        {
            ...
            if (!string.IsNullOrEmpty(saltBase64)) { _dynamicSalt = Convert.FromBase64String(saltBase64); return; }
        }
        _dynamicSalt = GenerateDynamicSalt();
        await SaveSaltAsync(_dynamicSalt, SaltVersion);
    }

    private static byte[] GenerateDynamicSalt() { var salt = new byte[32]; using var rng = RandomNumberGenerator.Create(); rng.GetBytes(salt); return salt; }

    private async Task SaveSaltAsync(byte[] salt, int version)
    {
        var cmd = _connection!.CreateCommand();
        cmd.CommandText = @"INSERT OR REPLACE INTO Metadata (Key, Value) VALUES ('SaltVersion', $version); INSERT OR REPLACE INTO Metadata (Key, Value) VALUES ('Salt', $salt)";
    ...
    public async Task SaveUsageAsync(UsageRecord usage)
    {
        ...
        var cmd = _connection.CreateCommand();
        cmd.CommandText = @"INSERT OR REPLACE INTO Usage (Id, Model, PromptTokens, CompletionTokens, TotalTokens, Timestamp, Source, EncryptedData) VALUES ($id, $model, $prompt, $completion, $total, $timestamp, $source, $encrypted)";
    ...
    public async Task<UsageRecord?> GetUsageAsync(string id)
    {
        ...
        if (await reader.ReadAsync()) return new UsageRecord { Id = reader.GetString(0), Model = reader.GetString(1), PromptTokens = reader.GetInt32(2), CompletionTokens = reader.GetInt32(3), TotalTokens = reader.GetInt32(4), Timestamp = DateTime.Parse(reader.GetString(5)), Source = reader.IsDBNull(6) ? null : reader.GetString(6) };
        return null;
    }
    ...
    public async Task<List<UsageRecord>> GetUsageByTimeRangeAsync(DateTime start, DateTime end)
    {
        ...
        using var reader = await cmd.ExecuteReaderAsync();
        while (await reader.ReadAsync()) results.Add(new UsageRecord { Id = reader.GetString(0), Model = reader.GetString(1), PromptTokens = reader.GetInt32(2), CompletionTokens = reader.GetInt32(3), TotalTokens = reader.GetInt32(4), Timestamp = DateTime.Parse(reader.GetString(5)), Source = reader.IsDBNull(6) ? null : reader.GetString(6) });
        return results;
    }
    ...
    private byte[] GetKey(byte[]? salt = null, bool isLegacy = false)
    {
        lock (_keyLock)
        {
            if (_cachedKey != null && salt == null && !isLegacy) return _cachedKey;
            byte[] derivedSalt;
            if (isLegacy) derivedSalt = LegacySalt;
            else if (salt != null) derivedSalt = salt;
            else if (_dynamicSalt != null) derivedSalt = _dynamicSalt;
            else { using var sha256 = SHA256.Create(); var passwordHash = sha256.ComputeHash(Encoding.UTF8.GetBytes(_password)); derivedSalt = new byte[16]; for (int i = 0; i < 16; i++) derivedSalt[i] = (byte)(LegacySalt[i % LegacySalt.Length] ^ passwordHash[i]); }
            var key = Rfc2898DeriveBytes.Pbkdf2(_password, derivedSalt, 100000, HashAlgorithmName.SHA256, 32);
            if (salt == null && !isLegacy) _cachedKey = key;
            return key;
        }
    }
    ...
    public async ValueTask DisposeAsync() { if (_connection != null) await _connection.DisposeAsync(); }
}