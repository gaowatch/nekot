...
{
    private readonly Dictionary<string, BrowserTab> _tabs = new();
    ...
    {
        var tab = new BrowserTab { Url = url, Title = url };
        _tabs[tab.Id] = tab;
        _activeTabId = tab.Id;
        return tab;
    }
    ...
    public void CloseTab(string id)
    {
        if (_tabs.Remove(id))
        {
            if (_activeTabId == id)
            {
                _activeTabId = _tabs.Keys.FirstOrDefault();
            }
        }
    }
    ...
    public BrowserTab? GetTab(string id)
    {
        return _tabs.TryGetValue(id, out var tab) ? tab : null;
    }
}
...
public class BrowserUrlValidator
{
    ...
    private static readonly HashSet<string> KeywordBlacklist = new(StringComparer.OrdinalIgnoreCase)
    {
        "cryptocurrency",
        "crypto.com",
        "miningpool",
    ...
    public bool IsAllowed(string url)
    {
        ...
        {
            var host = uri.Host;
            foreach (var blocked in DomainBlacklist)
            {
