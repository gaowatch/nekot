...
public partial class SettingsWindow : Window
{
    ...
    {
        if (sender is RadioButton radioButton && radioButton.Tag is string panelName) SwitchPanel(panelName);
    }

    private void SwitchPanel(string panelName)
    {
        foreach (var (name, setVisibility) in _panelVisibilityActions) setVisibility(name == panelName);
    }
    ...
    private async void OnSave(object? sender, RoutedEventArgs e)
    {
        try
        {
            ...
            if (_languageChanged)
            {
                ...
                if (result)
                {
                    ...
                    {
                        var startInfo = new ProcessStartInfo { FileName = exePath, Arguments = "--restart", UseShellExecute = true, WorkingDirectory = Path.GetDirectoryName(exePath) };
                        try { Process.Start(startInfo); Thread.Sleep(1000); }
                        catch (Exception ex) { System.Diagnostics.Debug.WriteLine($"Failed to restart: {ex.Message}"); }
                    }
        ...
        catch (Exception ex) { await ShowMessageAsync(Res.Common_Error, $"{Res.Settings_SaveFailed}: {ex.Message}"); }
    }
    ...
    {
        if (LanguageComboBox == null || LanguageComboBox.SelectedIndex < 0) return;
        var newLanguage = LanguageComboBox.SelectedIndex == 1 ? "en" : "zh-CN";
        var currentLanguage = UserSettingsService.Instance.Language;
        if (newLanguage != currentLanguage) _languageChanged = true;
    }
    ...
    private async void OnExportChatHistory(object? sender, RoutedEventArgs e)
    {
        try
        {
            ...
            var file = await storageProvider.SaveFilePickerAsync(new FilePickerSaveOptions
            {
                ...
                FileTypeChoices = new[] { new FilePickerFileType(Res.Export_JSON) { Patterns = new[] { "*.json" } } }
            });
        ...
        catch (Exception ex) { await ShowMessageAsync(Res.Common_Error, $"{Res.Export_ChatHistoryFailed}: {ex.Message}"); }
    }
    ...
    private async void OnExportTokenUsage(object? sender, RoutedEventArgs e)
    {
        ...
        catch (Exception ex) { await ShowMessageAsync(Res.Common_Error, $"{Res.Export_TokenUsageFailed}: {ex.Message}"); }
    }
    ...
    private async void OnImportData(object? sender, RoutedEventArgs e)
    {
        try
        {
            ...
            {
                Title = Res.Import_Data, AllowMultiple = false,
                FileTypeFilter = new[] { new FilePickerFileType(Res.Export_JSON) { Patterns = new[] { "*.json" } } }
            });
            var file = files.FirstOrDefault();
            if (file != null)
            {
                var result = await ShowConfirmAsync(Res.Import_ConfirmTitle, Res.Import_ConfirmMessage);
                if (result) { await ImportDataAsync(file.Path.LocalPath); LoadSettings(); await ShowMessageAsync(Res.Common_Success, Res.Import_Success); }
            }
        }
        catch (Exception ex) { await ShowMessageAsync(Res.Common_Error, $"{Res.Import_Failed}: {ex.Message}"); }
    }
    ...
    private async void OnClearAllData(object? sender, RoutedEventArgs e)
    {
        try
        {
            var result = await ShowConfirmAsync(Res.Clear_ConfirmTitle, Res.Clear_ConfirmMessage);
            if (result) { ClearAllData(); await ShowMessageAsync(Res.Common_Success, Res.Clear_Success); Close(); }
        }
        catch (Exception ex) { await ShowMessageAsync(Res.Common_Error, $"{Res.Clear_Failed}: {ex.Message}"); }
    }

    private void OnOpenGitHub(object? sender, RoutedEventArgs e)
    {
        try { Process.Start(new ProcessStartInfo { FileName = "https://github.com/nekot-ai/nekot", UseShellExecute = true }); }
        catch (Exception ex) { System.Diagnostics.Debug.WriteLine($"Failed to open GitHub: {ex.Message}"); }
    }
    ...
    private async Task ExportChatHistoryAsync(string filePath)
    {
        ...
        if (File.Exists(chatHistoryPath)) File.Copy(chatHistoryPath, filePath, true);
        else await File.WriteAllTextAsync(filePath, "[]");
    }
    ...
    private async Task ExportTokenUsageAsync(string filePath)
    {
        ...
        if (File.Exists(tokenUsagePath)) File.Copy(tokenUsagePath, filePath, true);
        else await File.WriteAllTextAsync(filePath, "[]");
    }
    ...
    private async Task ImportDataAsync(string filePath)
    {
        ...
        if (importedSettings != null) UserSettingsService.Instance.ImportSettings(importedSettings);
    }
    ...
    private void ClearAllData()
    {
        ...
        foreach (var file in new[] { "chat_history.json", "token_usage.json", "api_keys.json" })
        {
            var path = Path.Combine(appDataPath, file);
    ...
    private void UpdateVersionInfo()
    {
        ...
        catch { VersionText.Text = Res.Settings_Version_Unknown; }
    }
    ...
    private async Task ShowMessageAsync(string title, string message) { ErrorDialog.ShowError(this, title, message, null, ErrorSeverity.Info); await Task.CompletedTask; }

    private async Task<bool> ShowConfirmAsync(string title, string message)
    {
        var tcs = new TaskCompletionSource<bool>();
        var panel = new Panel { Background = new Avalonia.Media.SolidColorBrush(Avalonia.Media.Color.Parse("#80000000")) };
        var border = new Border
        {
            ...
            BorderThickness = new Thickness(1), CornerRadius = new CornerRadius(8), Padding = new Thickness(24),
            Width = 340, VerticalAlignment = Avalonia.Layout.VerticalAlignment.Center, HorizontalAlignment = Avalonia.Layout.HorizontalAlignment.Center
        };
        var stackPanel = new StackPanel { Spacing = 16 };
        var titleBlock = new TextBlock { Text = title, FontSize = 18, FontWeight = Avalonia.Media.FontWeight.Bold, HorizontalAlignment = Avalonia.Layout.HorizontalAlignment.Center, Foreground = new Avalonia.Media.SolidColorBrush(Avalonia.Media.Color.Parse("#FFFFFF")) };
        var messageBlock = new TextBlock { Text = message, FontSize = 14, TextWrapping = Avalonia.Media.TextWrapping.Wrap, HorizontalAlignment = Avalonia.Layout.HorizontalAlignment.Center, Foreground = new Avalonia.Media.SolidColorBrush(Avalonia.Media.Color.Parse("#B0B0B0")) };
        var buttonPanel = new StackPanel { Orientation = Avalonia.Layout.Orientation.Horizontal, HorizontalAlignment = Avalonia.Layout.HorizontalAlignment.Center, Spacing = 12 };
        var cancelButton = new Button { Content = Res.Common_Cancel, Classes = { "Secondary" }, Padding = new Thickness(16, 8) };
        var confirmButton = new Button { Content = Res.Common_Confirm, Classes = { "Primary" }, Padding = new Thickness(24, 8) };
        cancelButton.Click += (s, e) => { panel.IsVisible = false; if (this.Content is Panel rootPanel) rootPanel.Children.Remove(panel); tcs.SetResult(false); };
        confirmButton.Click += (s, e) => { panel.IsVisible = false; if (this.Content is Panel rootPanel) rootPanel.Children.Remove(panel); tcs.SetResult(true); };
        ...
        if (this.Content is Panel rootPanel) rootPanel.Children.Add(panel);
