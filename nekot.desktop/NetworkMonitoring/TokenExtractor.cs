...
public static class TokenExtractor
{
    ...
    public static TokenExtractedEventArgs? ExtractAuthFromHeaders(JsonElement headers, string url)
    {
        ...
        var args = new TokenExtractedEventArgs { Provider = provider, RequestUrl = url, Timestamp = DateTime.Now, TokenType = "Unknown" };
    ...
    public static TokenExtractedEventArgs? ExtractTokensFromResponse(string responseBody, string url)
    {
        ...
        try
        {
            ...
            var provider = DetectProvider(url) ?? "Unknown";
            var args = new TokenExtractedEventArgs { Provider = provider, RequestUrl = url, Timestamp = DateTime.Now };

            if (provider == "Doubao" && root.TryGetProperty("e", out var errorCode) &&
                root.TryGetProperty("sc", out var sourceChars) && root.TryGetProperty("tc", out var targetChars))
            {
            ...
            if (root.TryGetProperty("usage", out var usage)) ExtractTokensFromUsageObject(usage, ref totalTokens, ref promptTokens, ref completionTokens);
            if (totalTokens == 0 && root.TryGetProperty("token_count", out var tokenCount))
            {
                if (tokenCount.ValueKind == JsonValueKind.Object) ExtractTokensFromUsageObject(tokenCount, ref totalTokens, ref promptTokens, ref completionTokens);
                else if (tokenCount.ValueKind == JsonValueKind.Number) totalTokens = tokenCount.GetInt32();
            }
            if (totalTokens == 0 && root.TryGetProperty("statistics", out var statistics) &&
                statistics.TryGetProperty("usage", out var statsUsage)) ExtractTokensFromUsageObject(statsUsage, ref totalTokens, ref promptTokens, ref completionTokens);
            if (totalTokens == 0 && root.TryGetProperty("input_tokens", out var inputTokens))
            {
                promptTokens = inputTokens.GetInt32();
                if (root.TryGetProperty("output_tokens", out var outputTokens)) completionTokens = outputTokens.GetInt32();
            ...
            if (root.TryGetProperty("model", out var modelElem)) args.Model = modelElem.GetString();
            if (args.Tokens > 0) return args;
            return null;
        }
    ...
    private static class TokenEstimator
    {
        ...
        public static int EstimateTokensFromText(string? text)
        {
            ...
            {
                if (IsChineseCharacter(c)) chineseCount++;
                else if (char.IsLetter(c)) englishCount++;
                else if (char.IsDigit(c)) numberCount++;
                else if (!char.IsWhiteSpace(c)) symbolCount++;
            }
            var estimated = chineseCount / ChineseCharsPerToken + englishCount / EnglishCharsPerToken +
                            numberCount / NumberCharsPerToken + symbolCount / SymbolCharsPerToken;
    ...
    private static class SecureSaltManager
    {
        ...
        private static readonly string SaltFilePath = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "NekoT", "token_hash_salt.bin");
