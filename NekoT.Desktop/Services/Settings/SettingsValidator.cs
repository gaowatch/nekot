...
public class SettingsValidator : ISettingsValidator
{
    ...
    private ValidationResult ValidateUrl(string? url, string propertyName, int maxLength)
    {
        ...
        {
            return ValidationResult.Failure(propertyName, string.Format(Res.Validation_UrlTooLong, maxLength));
        }
        ...
        foreach (var protocol in DangerousProtocols)
        {
            ...
            {
                return ValidationResult.Failure(propertyName, string.Format(Res.Validation_UrlInvalidProtocol, protocol));
            }
        }

        if (!Uri.TryCreate(url, UriKind.Absolute, out var uri))
        {
            return ValidationResult.Failure(propertyName, Res.Validation_UrlInvalidFormat);
        }
        ...
        {
            return ValidationResult.Failure(propertyName, Res.Validation_ProtocolNotSupported);
        }
    ...
    private ValidationResult ValidateProxyUrl(string? url)
    {
        ...
        if (url.Length > 512)
        {
            return ValidationResult.Failure("ProxyUrl", Res.Validation_ProxyUrlTooLong);
        }
        ...
        foreach (var protocol in DangerousProtocols)
        {
            if (lowerUrl.StartsWith(protocol))
            {
                return ValidationResult.Failure("ProxyUrl", string.Format(Res.Validation_ProxyUrlInvalidProtocol, protocol));
            }
        }

        if (!Uri.TryCreate(url, UriKind.Absolute, out var uri))
        {
            return ValidationResult.Failure("ProxyUrl", Res.Validation_ProxyUrlInvalidFormat);
        }
        ...
        {
            return ValidationResult.Failure("ProxyUrl", Res.Validation_ProxyProtocolNotSupported);
        }
    ...
    private ValidationResult ValidateUserAgent(string? userAgent)
    {
        ...
        {
            return ValidationResult.Failure("UserAgent", Res.Validation_UserAgentTooLong);
        }

        var dangerousChars = new[] { '<', '>', '"', '\'', '\n', '\r', '\0' };
        if (userAgent.IndexOfAny(dangerousChars) >= 0)
        {
            return ValidationResult.Failure("UserAgent", Res.Validation_UserAgentInvalidChars);
        }
    ...
    private string? SanitizeUrl(string? url)
    {
        ...
        return url.Trim().Replace("\0", string.Empty);
    }
