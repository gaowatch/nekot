...
public partial class BrowserTabView : UserControl
{
    ...
    protected override void OnAttachedToVisualTree(Avalonia.VisualTreeAttachmentEventArgs e)
    {
        ...
        if (_isInitialized && _attachedWebView != null && _isWebViewReady) { Log("[BrowserTabView] Already initialized"); return; }
        if (_isInitialized && _attachedWebView != null && !_isWebViewReady) { _isInitialized = false; _attachedWebView = null; }
    ...
    protected override void OnDetachedFromVisualTree(Avalonia.VisualTreeAttachmentEventArgs e)
    {
        ...
        if (_attachedWebView != null)
        {
            ...
            catch (Exception ex) { Log($"[BrowserTabView] Error unsubscribing events: {ex.Message}"); }
    ...
    private void OnWebViewInitializationFailed(object? sender, string reason) { Log($"[BrowserTabView] WebView init failed: {reason}"); _isWebViewReady = false; }
    ...
    {
        if (e.Key == Key.Enter && DataContext is BrowserTabViewModel viewModel) { viewModel.Navigate(); e.Handled = true; }
    }

    private void OnNavigateClick(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        if (DataContext is BrowserTabViewModel viewModel) viewModel.Navigate();
    }
}