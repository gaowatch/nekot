...
public static class WindowIconHelper
{
    ...
    [DllImport("user32.dll", SetLastError = true)] private static extern bool SetWindowPos(IntPtr hWnd, IntPtr hWndInsertAfter, int x, int y, int width, int height, uint uFlags);
    ...
    public static void RemoveIcon(Window window)
    {
        ...
        window.Opened += (sender, e) => { try { RemoveIconInternal(window); } catch (Exception ex) { System.Diagnostics.Debug.WriteLine($"移除窗口图标失败: {ex.Message}"); } };
    }
    ...
    public static void RestoreIcon(Window window)
    {
        if (window == null) throw new ArgumentNullException(nameof(window));
        if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows)) return;
        window.Opened += (sender, e) => { try { RestoreIconInternal(window); } catch { } };
    }
