...
public class WebView2Control : NativeControlHost
{
    ...
    private void OnCoreWebView2DocumentTitleChanged(object? s, object args)
    {
        if (_coreWebView2 != null)
        {
            var title = _coreWebView2.DocumentTitle;
            DocumentTitleChanged?.Invoke(this, title);
            Logger.LogInfo(LogCategory, $"Document title changed: {title}");
        }
    }
    
    private void OnCoreWebView2NavigationStarting(object? s, CoreWebView2NavigationStartingEventArgs args)
    {
        if (_controller != null)
        {
            _controller.DefaultBackgroundColor = System.Drawing.Color.FromArgb(
    ...
    private void OnCoreWebView2ContentLoading(object? s, CoreWebView2ContentLoadingEventArgs args)
    {
        Logger.LogInfo(LogCategory, $"Content loading, IsErrorPage={args.IsErrorPage}");
    }
    ...
    private void OnCoreWebView2NavigationCompleted(object? s, CoreWebView2NavigationCompletedEventArgs args)
    {
        ...
        if (!args.IsSuccess)
        {
            if (_errorPageRetryCount >= MaxErrorPageRetries)
            {
                Logger.LogInfo(LogCategory, $"Max error page retries ({MaxErrorPageRetries}) reached, stopping");
                return;
            }
    ...
    private void OnCoreWebView2NewWindowRequested(object? s, CoreWebView2NewWindowRequestedEventArgs args)
    {
        ...
        {
            Logger.LogInfo(LogCategory, "NewWindowRequested: URI is empty, cancelling");
            return;
        }
        
        if (!Uri.TryCreate(args.Uri, UriKind.Absolute, out var uri))
        {
            Logger.LogInfo(LogCategory, $"NewWindowRequested: Invalid URI format: {args.Uri}");
            return;
        }
        ...
        {
            Logger.LogInfo(LogCategory, $"NewWindowRequested: Blocked disallowed scheme '{uri.Scheme}' for URI: {args.Uri}");
            return;
        }
    ...
    private async Task ApplyStealthModeAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                Logger.LogInfo(LogCategory, "Stealth mode disabled by user");
                return;
            }
            ...
            {
                _coreWebView2.Settings.UserAgent = userAgent;
                Logger.LogInfo(LogCategory, $"User-Agent set: {userAgent[..Math.Min(50, userAgent.Length)]}...");
            }
            else
            {
                _coreWebView2.Settings.UserAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36";
                Logger.LogInfo(LogCategory, "User-Agent set to default Chrome");
            }
            ...
            if (cancellationToken.IsCancellationRequested)
            {
                Logger.LogInfo(LogCategory, "ApplyStealthModeAsync CANCELLED before script injection");
                return;
            }
            ...
            {
                var trackingProtectionScript = GetTrackingProtectionScript();
                await _coreWebView2.AddScriptToExecuteOnDocumentCreatedAsync(trackingProtectionScript);
                Logger.LogInfo(LogCategory, "Tracking protection script injected");
            }
            
            if (settings.BlockAds)
            {
                var adBlockScript = GetAdBlockScript();
                await _coreWebView2.AddScriptToExecuteOnDocumentCreatedAsync(adBlockScript);
                Logger.LogInfo(LogCategory, "Ad blocking script injected");
            }

            var lightBackgroundCss = $@"
                html, body {{
                    background-color: {AppConstants.WebView2Theme.LightBackgroundColorHex} !important;
                    background: {AppConstants.WebView2Theme.LightBackgroundColorHex} !important;
                }}
            ";
        ...
        {
            Logger.LogInfo(LogCategory, $"ApplyStealthModeAsync failed: {ex.Message}");
        }
    }

    private static string GetTrackingProtectionScript()
    {
        return @"
            (function() {
                'use strict';
    ...
    protected override void DestroyNativeControlCore(IPlatformHandle control)
    {
        ...
        if (_state == WebViewState.Initializing)
        {
            Logger.LogInfo(LogCategory, "DestroyNativeControlCore: WebView2 is initializing, cancel it!");
        ...
        if (_controller != null)
        {
            try
            {
                var closeTask = Task.Run(() => _controller.Close());
                if (!closeTask.Wait(TimeSpan.FromSeconds(3)))
                {
                    Logger.LogInfo(LogCategory, "Controller.Close() timeout after 3 seconds, forcing cleanup");
                }
                else
                {
                    Logger.LogInfo(LogCategory, "Controller.Close() completed successfully");
                }
            }
            catch (Exception ex)
            {
                Logger.LogInfo(LogCategory, $"Error closing controller: {ex.Message}");
            }
            _controller = null;
        }
    ...
    public void Navigate(string url)
    {
        ...
        {
            Logger.LogInfo(LogCategory, "Navigate FAILED: URL is null or empty");
            ErrorOccurred?.Invoke(this, "URL 不能为空");
            return;
        }
        ...
        switch (_state)
        {
            ...
            case WebViewState.Initializing:
                _pendingUrl = url;
                Logger.LogInfo(LogCategory, $"Navigate DEFERRED: WebView2 is initializing, saved pending URL");
                break;
            ...
            case WebViewState.NotInitialized:
                _pendingUrl = url;
                StartInitialization();
                break;
                
            case WebViewState.Disposed:
                Logger.LogInfo(LogCategory, "Navigate FAILED: Control is disposed");
                ErrorOccurred?.Invoke(this, "控件已销毁");
                break;
        }
    }
    ...
    public async Task<bool> NavigateAsync(string url, CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(url))
        {
            Logger.LogInfo(LogCategory, "NavigateAsync FAILED: URL is null or empty");
            ErrorOccurred?.Invoke(this, "URL 不能为空");
            return false;
        }
        ...
        if (_state == WebViewState.Disposed)
        {
            Logger.LogInfo(LogCategory, "NavigateAsync FAILED: Control is disposed");
            ErrorOccurred?.Invoke(this, "控件已销毁");
            return false;
        }
        ...
        try
        {
            if (_state == WebViewState.Initializing && _initCompletionSource != null)
            {
                ...
                using var linkedCts = CancellationTokenSource.CreateLinkedTokenSource(
                    cancellationToken, 
                    _initCancellationTokenSource?.Token ?? CancellationToken.None);
                
                var completed = await _initCompletionSource.Task.WaitAsync(linkedCts.Token);
                
                if (!completed || _state != WebViewState.Ready)
                {
                    Logger.LogInfo(LogCategory, $"NavigateAsync FAILED: Initialization did not complete successfully, State={_state}");
                    return false;
                }
            }
            else if (_state == WebViewState.Failed || _state == WebViewState.NotInitialized)
            {
                ...
                {
                    Logger.LogInfo(LogCategory, "NavigateAsync FAILED: Cannot start initialization");
                    return false;
                }
                
                if (_initCompletionSource == null)
                {
                    Logger.LogInfo(LogCategory, "NavigateAsync FAILED: InitCompletionSource is null");
                    return false;
                }
                ...
                if (!completed || _state != WebViewState.Ready)
                {
                    Logger.LogInfo(LogCategory, $"NavigateAsync FAILED: Initialization failed, State={_state}");
                    return false;
                }
            }
        ...
        {
            Logger.LogInfo(LogCategory, "NavigateAsync CANCELLED");
            return false;
        }
        catch (Exception ex)
        {
            Logger.LogInfo(LogCategory, $"NavigateAsync EXCEPTION: {ex.Message}");
            return false;
        }
    }
    ...
    private void NavigateInternal(string url)
    {
        if (_coreWebView2 == null)
        {
            Logger.LogInfo(LogCategory, "NavigateInternal FAILED: CoreWebView2 is null");
            ErrorOccurred?.Invoke(this, "WebView2 未初始化");
            return;
        }
        ...
        try
        {
            ...
            {
                Logger.LogInfo(LogCategory, $"NavigateInternal BLOCKED: Dangerous protocol in URL");
                ErrorOccurred?.Invoke(this, "不允许的协议");
                return;
            }
        ...
        {
            Logger.LogInfo(LogCategory, $"Navigate FAILED: {ex.Message}");
            ErrorOccurred?.Invoke(this, $"导航失败: {ex.Message}");
        }
    }
    ...
    private bool StartInitialization()
    {
        if (_state == WebViewState.Disposed)
        {
            Logger.LogInfo(LogCategory, "StartInitialization FAILED: Control is disposed");
            return false;
        }
        
        if (_state == WebViewState.Initializing)
        {
            Logger.LogInfo(LogCategory, "StartInitialization SKIPPED: Already initializing");
            return true;
        }
        
        if (_hwnd == IntPtr.Zero)
        {
            Logger.LogInfo(LogCategory, "StartInitialization FAILED: HWND is zero");
            return false;
        }
        ...
        _ = InitializeWebView2Async(_initCancellationTokenSource.Token).ContinueWith(
            task => 
            {
                if (task.Exception != null)
                {
                    Logger.LogInfo(LogCategory, $"InitializeWebView2 unhandled exception: {task.Exception}");
                }
            }, 
    ...
    private void TryReinitialize()
    {
        ...
        {
            Logger.LogInfo(LogCategory, $"TryReinitialize FAILED: Max retries ({MaxInitRetries}) reached");
            ErrorOccurred?.Invoke(this, $"初始化失败次数过多，请重启应用。原因: {_initFailureReason}");
            return;
        }
        ...
        _ = Task.Run(async () =>
        {
            await Task.Delay(InitRetryDelayMs);
            
            if (_state == WebViewState.Failed || _state == WebViewState.NotInitialized)
            {
                StartInitialization();
            }
        });
    ...
    private static string GetHttpErrorMessage(int httpStatusCode)
    {
        return httpStatusCode switch
        {
            ...
            503 => "服务不可用",
            504 => "网关超时",
            _ when httpStatusCode >= 400 && httpStatusCode < 500 => $"客户端错误 ({httpStatusCode})",
            _ when httpStatusCode >= 500 => $"服务器错误 ({httpStatusCode})",
            _ => $"HTTP错误 ({httpStatusCode})"
        };
    }
    ...
    private void ShowErrorPage(string errorMessage, string url)
    {
        ...
        if (_coreWebView2 == null)
        {
            Logger.LogInfo(LogCategory, "ShowErrorPage ABORTED: _coreWebView2 is null");
            return;
        }
...
<head>
    ...
    <style>
        :root {{
            --bg: {AppConstants.WebView2Theme.LightBackgroundColorHex};
            --fg: #000000;
            --muted: #666666;
            --accent: #0078D4;
            --card: #F5F5F5;
...
<body>
    <div class=""container"">
        <div class=""brand"">NekoT</div>
        
        <div class=""error-code"">://</div>
        <div class=""error-title"">无法访问此页面</div>
        <div class=""error-message"">{safeMessage}</div>
        
        <div class=""url-box"">{safeUrl}</div>
        
        <div class=""help-text"">
            点击左上角的 <span class=""highlight"">刷新按钮</span> 重新加载
        </div>
    </div>
</body>
</html>";
    ...
    private void OnWebMessageReceived(object? sender, CoreWebView2WebMessageReceivedEventArgs e)
    {
        ...
        Logger.LogInfo(LogCategory, $"WebMessage received: {message}");
